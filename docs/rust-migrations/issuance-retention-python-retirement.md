...
Status: locally qualified for the two-route Python deletion, not yet merged or
deployed. The two native retention routes landed through exact-head and
merge-group gates in `ElevenID/marty-ui#849` at protected-main commit
`91ea7e52d6a3822981bb42f002da8c68532ebf2e`, with its published-image
dependency pinned. The prior OID4VCI/Canvas retirement
landed as `ElevenID/marty-credentials#298` at
`af041c2c3bd2e9fae189b566499146f97156120f`, with a qualified gate. This
separate retention qualification pins four SHA-256 values from committed Git
blobs, not platform-converted checkout files. The verifier passed against the
live protected UI main and authorized exactly two deletions while retaining all
nine physical-passport routes.
...
Local preparation gates: 1,648 unit tests passed, five skipped, with 200
...
A fresh hosted dependency gate remains required before merge. After the
protected UI merge, the focused retirement and qualification tests passed
11/11 and the live-source verifier returned `qualified` with two authorized
deletions and nine retained Python routes. The required `CI Gate` now includes
a dedicated provenance lane that checks out that exact UI merge commit,
refreshes live protected `main`, and reruns the verifier before this deletion
can merge.
