...
def test_legacy_format_is_ignored_and_cannot_replace_the_final_selector() -> None:
    legacy_only = CredentialRequest.model_validate({"format": "vc+sd-jwt"})
    assert legacy_only.credential_configuration_id is None
    assert legacy_only.credential_identifier is None
    assert not hasattr(legacy_only, "format")
