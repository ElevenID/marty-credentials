...
pub(crate) fn register_mdoc_module(parent: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    parent.add_function(wrap_pyfunction!(verification::parse_device_response, parent)?)?;
    parent.add_function(wrap_pyfunction!(verification::verify_mdoc_cbor, parent)?)?;
    parent.add_function(wrap_pyfunction!(verification::verify_mdoc_signature, parent)?)?;
